...
class Certi extends \Magento\Framework\App\Action\Action {
        ...
        try {
            ...
            if(strtolower($post['storename'])== "italiano"){
             ...
             if($error['return'] == "KO"){
                ...
                if($lang == "it"){
                  $response = ['equipment_status' => 'false','message' => "Il Codice Equipment errato" ];
                }else{
                $response = ['equipment_status' => 'false','message' => " The Equipment Code is Incorrect" ];
                }
                return $resultJson->setData($response);
     ...
     public function checkEquipmentApi($code ,$mainbrand, $storename)
    {
        ...
        if(strtolower($mainbrand) == "bliss" || strtolower($mainbrand) == "calderoni"){
            $url1 = 'https://testapp.damianigroup.com/wsservice/services/damiani-json/execute?action=GET_MATERIAL_INFO&';
            $postData = array(
            ...
            $url1 = 'https://testapp.damianigroup.com/wsservice/services/damiani-json/execute?action=GET_EQUIPMENT_INFO&';
                $postData = array(
