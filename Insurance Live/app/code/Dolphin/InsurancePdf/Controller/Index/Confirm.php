...
class Confirm extends \Magento\Framework\App\Action\Action
{
	...
	public function execute()
	{
        ...
        if($lang == "it"){
		 $ID_MODULE = "a17a81253ea5";
        }

        if($lang == "it" && strtolower($postUpdate->getBrand()) == "calderoni"){
         $ID_MODULE = "a17aa1ba0106";
        }  

        if($lang == "en" && strtolower($postUpdate->getBrand()) == "calderoni"){
         $ID_MODULE = "a17a9f97365c";
        }  
    ...
    public function Emailexist($certiArray)
    {
      ...
      /*$certiChangeCollectionSave = $this->certificatoFactory->create()->load($certiArray['certificato_id']);
      $expire = date('Y-m-d', strtotime('+3 years'));
      $finalExpire =  date('Y-m-d', strtotime('-1 day', strtotime($expire)));
      $certiChangeCollectionSave->setStatus('1');
      $certiChangeCollectionSave->setExpireDate($finalExpire);
