<?php
namespace Luxmadein\Certificatodisostituzione\Model;

class CronBrowserTest extends \Magento\Framework\Model\AbstractModel {
	...
	public function getCronBrowser() {
		...
		$date = (new \DateTime())->modify('-0 day');
		...
		$orderDatamodel->addFieldToFilter('main_table.updated_at', ['lteq' => $date->format('Y-m-d 23:59:59')])
			...
			->addFieldToFilter('main_table.status', ['eq' => "processing"]);
		...
		foreach ($orderDatamodel->getData() as $orderData) {
			...
			$sender = [
				...
				'email' => $this->_escaper->escapeHtml("noreply@bliss.it"),
			];

			$storeScope = \Magento\Store\Model\ScopeInterface::SCOPE_STORE;
			$transport = $this->_transportBuilder
				->setTemplateIdentifier('luxmadein_certificatodisostituzione_email_template_it')
	            ->setTemplateOptions(
	                [
	                    'area' => "frontend",
	                    'store' => 1,
				...
				->setFrom($sender)
				->addTo("divyeshdusara007@yopmail.com")
				//->addCc("divyesh@dolphinwebsolution.com")
				->getTransport();
			/*echo "<pre>";
			var_dump($transport);
			die;*/ 
