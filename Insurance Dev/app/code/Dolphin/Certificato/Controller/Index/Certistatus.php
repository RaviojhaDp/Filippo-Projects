...
class Certistatus extends \Magento\Framework\App\Action\Action {

protected $sessionFactory;
    public function __construct(
    \Magento\Framework\App\Action\Context $context, \Magento\Framework\ObjectManagerInterface $objectmanager, \Dolphin\Certificato\Model\CertificatoFactory $certificatoModelFactory, \Magento\Customer\Model\SessionFactory $sessionFactory
    ) {
    ...
    public function execute() {
        ...
        $model->addFieldToFilter('certificato_code', array('eq' => $data));
        ...
        if (count($model->getData()) > 0) {
            ...
            ?></span></p>
                <p><?php echo __('Expire Date'); ?></br><span><?php echo date("d-m-Y", strtotime(@$model[0]['expire_date']));
            ?></span></p> 
                        <?php if (strtolower($cat_name) == 'damiani' && $difference > 1 && $months <= 6 && $years == 0) { ?>
                    <div class="Renewal Certificato"><button id="renewal_new"><a href="<?php echo $base_url_config.strtolower($cat_name); ?>/assicurazione.html?<?php echo "certid=" . $model[0]['certificato_id']; ?>&renewal=1"><?php echo __('Renewal Certificate') ?></a></button></div>
                <?php } ?>
                ...
                <?php if (($difference > 1) && ($customer_group_id != '5')) { ?>
                    ...
                    if (strtolower($storename) == "italiano") {
                        echo __('Annullamento Assicurazione');
                    } else {
                        echo __('Cancel Warranty');
                    }
                    ?></button></div>
            <?php } ?> 
