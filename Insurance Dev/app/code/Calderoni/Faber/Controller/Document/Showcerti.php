...
class Showcerti extends \Magento\Framework\App\Action\Action
{
    ...
    $directoryList = $om->get('Magento\Framework\App\Filesystem\DirectoryList');
		...
		if($header_id){
			...
			if($headerData["status"]){
				...
				$resultPage = $this->resultFactory
					...
					->setHeader('Content-Disposition', "attachment; filename=\"".$post['certificato_code'].".pdf\"", true)
					->setContents($docx)
					;
        ...
        $media = $filesystem->getDirectoryWrite($directoryList::MEDIA);
         $path = "Signed_Document/Certificato/$header_id/".$post['certificato_code'].".pdf";
        //echo $path;
    $contents = $docx;
